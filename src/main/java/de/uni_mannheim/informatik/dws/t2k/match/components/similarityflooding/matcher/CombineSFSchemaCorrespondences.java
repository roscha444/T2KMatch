 ...
 */
public class CombineSFSchemaCorrespondences {
    ...
    public CombineSFSchemaCorrespondences(Processable<Correspondence<MatchableTableColumn, MatchableTableRow>> keyCorrespondences) {
    ...
    public Processable<Correspondence<MatchableTableColumn, MatchableTableRow>> run() {
        ...
        if (isVerbose()) {
            for (Pair<String, Correspondence<MatchableTableColumn, MatchableTableRow>> record : sum.get()) {
                System.out.println(String.format("[%b] (%.8f) %s <-> %s", record.getSecond().getSimilarityScore() >= finalPropertySimilarityThreshold, record.getSecond().getSimilarityScore(),
                    record.getSecond().getFirstRecord(), record.getSecond().getSecondRecord()));
            }
        }
